...
Curso ::Curso(string nombre, string Descripcion, Dificultad Complejidad, int CantLecciones, int cantEjericios, Idioma *idiomaC, Profesor *profesorC, list<Leccion *> Lecciones, set<Curso *> cursos_previos)
{
    setNombre(nombre);
...
int Curso ::getPromedioEstudiante(Estudiante *est)
{
...
map<string, int> Curso ::getProgreso_estudiantes()
{
...
void Curso ::actualizarPromedioCurso()
{
    // hacer
}
...
void Curso ::addInscripcion(Estudiante *est, Fecha fec)
{
    ...
    // //agregar curso a la lista de cursos del estudiante y a la lista de cursos incompletos del estudiante
    // est->addCurso(this);
    // est->addCursoIncompleto(this);
}
...
// dataInfoCurso * Curso :: crearDataInfoCurso() {
//     dataInfoCurso * res = new dataInfoCurso(this-> Nombre, this-> Descripcion, this-> Complejidad, this-> CantLecciones, this-> Habilitado, this-> Promedio, this-> idiomaC->getNombre(), this-> profesorC->getNombre());
//     return res;
// }
